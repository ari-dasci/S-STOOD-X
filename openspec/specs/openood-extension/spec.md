...
### Requirement: numpy.sctypes test-infrastructure shim and undeclared dep additions
...
MUST cite the `imgaug` version and upstream issue. Additionally, the OpenOOD pin at rev
`3c35632` imports `timm` and `foolbox` eagerly but does not declare them in its metadata
(upstream packaging bug); these MUST be added as explicit STOOD-X dependencies (`timm>=0.9`,
`foolbox>=3.3`) to unblock collection of `openood.attacks.misc` and
`openood.evaluation_api.attackdataset`. The combination of the conftest shim AND the
`timm`/`foolbox` dep additions together RESOLVES env R3 (previously DEFERRED).

#### Scenario: pytest collects all 7 modules with zero import errors

- GIVEN `uv sync` has completed AND the root `conftest.py` shim is present AND `timm`/`foolbox` are installed
