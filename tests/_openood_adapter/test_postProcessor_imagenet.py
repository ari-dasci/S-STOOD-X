...
def test_id_XAI_postProcessor(
    ...
    net = torchvision.models.__dict__[model_name](weights=weights, num_classes=num_classes[id_set])
    net.eval()
    config = {
        "K": K,
        "NN_K": NN_K,
...
def test_Baseline_postProcessor(id_set, model_name, weights, fsood, baseline_postprocessor):
    ...
    net = torchvision.models.__dict__[model_name](weights=weights, num_classes=num_classes[id_set]).to(device)
