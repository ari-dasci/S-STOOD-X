...
@pytest.mark.parametrize("id_set", ["imagenet200"])
...
@pytest.mark.parametrize(
    ...
    [
        [
            "resnet18",
            "./pretrained_models/imagenet200_resnet18_224x224_base_e90_lr0.1_default/s0/best.ckpt",
            "avgpool",
        ],
    ],
)
...
def test_id_XAI_imagenet200_postProcessor(
    id_set,
    model_name,
    weights,
    feature_name,
    ...
    net = ResNet18_224x224(num_classes=num_classes[id_set])
...
def test_Baseline_imagnet200_postProcessor(id_set, model_name, weights, fsood, baseline_postprocessor):
    ...
    net = ResNet18_224x224(num_classes=num_classes[id_set]).to(device)
    net.load_state_dict(torch.load(weights, map_location=device))
    net.eval()
    evaluator = Evaluator(
        net=net,
