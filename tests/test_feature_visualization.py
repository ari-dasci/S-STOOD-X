...
class ViT_B_16(VisionTransformer):
    def __init__(
        self, image_size=224, patch_size=16, num_layers=12, num_heads=12, hidden_dim=768, mlp_dim=3072, num_classes=1000
    ):
...
def test_FeatureExplanation(model_name, weights, id_set, feature_name, id_val, ood_type, ood_dataset, K):
    ...
    for i, example in enumerate(nearest_examples):
        save_tensor_as_image(example, f"visualization/ID/{id_val}/{model_name}/NN_{i}.png")
    n_features = 5
    ...
    for i in range(n_features):
        ...
        save_sum_tensor_and_heatmap(
            f"visualization/ID/{id_val}/{model_name}/feat_{i}_sum.png", x_, heatmap, cmap_index=i
        )
        ...
        for j, example in enumerate(nearest_examples):
            ...
            save_sum_tensor_and_heatmap(
                f"visualization/ID/{id_val}/{model_name}/feat_{i}_NN_{j}_sum.png", example, heatmap, cmap_index=i
            )
    ...
    for i, example in enumerate(nearest_examples):
        save_tensor_as_image(example, f"visualization/OOD/{ood_type}_{ood_dataset}/{id_val}/{model_name}/NN_{i}.png")
    main_features = feature_explainator.features_presence(x_.unsqueeze(0), k=n_features)
    max_heatmap = feature_explainator.presence_of_feature(x_, main_features[0:n_features])
    print("Save all-the-features map")
    save_heatmap_as_image(
    ...
    save_sum_tensor_and_heatmap(
        f"visualization/OOD/{ood_type}_{ood_dataset}/{id_val}/{model_name}/all_features_sum.png", x_, max_heatmap
    )
    for i in range(n_features):
        heatmap = feature_explainator.presence_of_feature(x_, main_features[i : i + 1])
        print(f"Heatmap of main feature {i}")
        save_heatmap_as_image(
            heatmap,
            f"visualization/OOD/{ood_type}_{ood_dataset}/{id_val}/{model_name}/feat_{i}_heatmap.png",
            cmap_index=i,
        )
        save_sum_tensor_and_heatmap(
            f"visualization/OOD/{ood_type}_{ood_dataset}/{id_val}/{model_name}/feat_{i}_sum.png",
            x_,
            heatmap,
            cmap_index=i,
        )
        ...
        for j, example in enumerate(nearest_examples):
            ...
            save_sum_tensor_and_heatmap(
                f"visualization/OOD/{ood_type}_{ood_dataset}/{id_val}/{model_name}/feat_{i}_NN_{j}_sum.png",
                example,
                heatmap,
                cmap_index=i,
            )
